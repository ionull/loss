			...
			if (file && [file isKindOfClass:[NSURL class]]) {
				if ([file isFileURL]) {
					...
					if ([fm fileExistsAtPath:path isDirectory:&isDir] && (!isDir)) {
						...
						if ([[[TZAppDelegate sharedAppDelegate] supportVideoFormats] containsObject:ext] ||
							[[[TZAppDelegate sharedAppDelegate] supportAudioFormats] containsObject:ext]) {
							// 如果是支持的格式
							[self playMedia:file];
							break;
							
						} else if ([[[TZAppDelegate sharedAppDelegate] supportSubFormats] containsObject:ext]) {
							// 如果是字幕文件
...
-(void) playMedia:(NSURL*)url
{
  ...
  [mplayer.pm setSmartSubMatching:[ud boolForKey:kUDKeySmartSubMatching]];
	...
	if ([ud boolForKey:kUDKeyAutoResume] && (stime = [[[TZAppDelegate sharedAppDelegate] bookmarks] objectForKey:[lastPlayedPathPre absoluteString]])) {
	...
	for (NSString *mediaFile in directoryEnumerator)
	{
		...
		if ([[fileAttr objectForKey:NSFileType] isEqualToString:NSFileTypeDirectory]) {
			...
			[directoryEnumerator skipDescendants];
					...
					([[[TZAppDelegate sharedAppDelegate] supportVideoFormats] containsObject:ext] ||
					 [[[TZAppDelegate sharedAppDelegate] supportAudioFormats] containsObject:ext])) {
			// 如果是正常文件，并且是媒体文件
...
-(void) togglePlayPause
{
	if (mplayer.state == kMPCStoppedState) {
		...
		if (lastPlayedPath) {
			...
			[[TZAppDelegate sharedAppDelegate] openFile:self];
...
{
	// 用文件名查找有没有之前的播放记录
	NSNumber *stopTime = [[[TZAppDelegate sharedAppDelegate] bookmarks] objectForKey:[lastPlayedPathPre absoluteString]];
	NSDictionary *dict;
...
{	
	...
	if (stoppedByForce) {
		...
		[[[TZAppDelegate sharedAppDelegate] bookmarks] setObject:[dict objectForKey:kMPCPlayStoppedTimeKey] forKey:[lastPlayedPath absoluteString]];
	} else {
		// 自然关闭
		// 删除这个文件key的播放时间
